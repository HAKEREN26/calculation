    ...
    <>
      ...
      <div className="pw">
        ...
        {showErrs && hasErrs && <div className="err-banner">
          <strong>⚠️ Please complete all required fields before continuing</strong>
          ...
          <div className="fc-body">
            ...
            {page===3&&<>
              ...
              {f.salaryIncreases.map((si,i)=><div key={i} className="sal-blk">
                <button className="rm" onClick={()=>rmSI(i)}>×</button>
                <div className="g2">
                  <F label={`New salary #${i+1} (₪)`} err={E(`si_${i}_sal`)}>
                    <input {...inp(`si_${i}_sal`)} type="number" min="0" value={si.newSal} onChange={e=>updSI(i,'newSal',e.target.value)} placeholder="₪"/>
                  </F>
              ...
              {f.vacations.map((v,i)=><div key={i} className="vac-blk">
                {f.vacations.length>1&&<button className="rm" onClick={()=>rmV(i)}>×</button>}
            ...
            {sendError&&<div className="err-banner" style={{marginTop:12}}>
              <strong>⚠️ Submission failed — please try again</strong>
              {sendError}
            </div>}
            <div className="nav">
