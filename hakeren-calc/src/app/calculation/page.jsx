...
const YN = [{v:'yes',l:'Yes',he:'כן'},{v:'no',l:'No',he:'לא'}];
...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .he{color:#5a7a9e;font-weight:400;unicode-bidi:isolate}
  .field label .he{font-size:12px}
  .rb .he,.chk-item .he{font-size:11px;margin-inline-start:4px}
...
export default function App() {
  ...
  const nats = ['Filipino','Romanian','Moldovan','Ukrainian','Russian','Chinese','Indian','Thai','Nepali','Georgian','Uzbek','Azerbaijani','Kazakh','Belarusian','Ethiopian','Eritrean','Nigerian','Kenyan','South African','Ugandan','Tanzanian','Sudanese','South Sudanese','Ghanaian','Colombian','Venezuelan','Brazilian','Argentinian','Bangladeshi','Myanmarese','Vietnamese','Turkish','Jordanian','Egyptian','Israeli','Sri Lankan','Peruvian','Ivorian','Haitian','Malawian','Other'];
          ...
          <div className="fc-body">
            ...
            {page===2&&<>
              ...
              <div className="g2">
                ...
                <F label="Employment End Date" he="תאריך סיום העסקה" req err={E('end')}>
              ...
              <div className="g2">
                ...
                <F label="Pocket money per month (NIS)" he="דמי כיס לחודש (בשקלים)" hint="Enter 0 if none">
              ...
              {f.salaryIncreases.map((si,i)=><div key={i} className="sal-blk">
                ...
                <div className="g2">
                  <F label={`New salary #${i+1} (NIS)`} he="שכר חדש (בשקלים)" err={E(`si_${i}_sal`)}>
                    <input {...inp(`si_${i}_sal`)} type="number" min="0" value={si.newSal} onChange={e=>updSI(i,'newSal',e.target.value)} placeholder="NIS"/>
                  </F>
                  <F label="Effective from date" he="תאריך תחילת התוקף" err={E(`si_${i}_date`)}>
              ...
              {f.worksWeekends==='yes'&&<>
                <F label="How many weekends in a month?" he="כמה סופי שבוע בחודש?">
                ...
                <F label="How much do you receive per Shabbat day? (NIS)" he="כמה את/ה מקבל/ת עבור יום שבת? (בשקלים)" hint="The amount your employer pays you for each Shabbat day worked — not the monthly total" err={E('shabat')} style={{marginTop:8}}>
              ...
              {f.vacations.map((v,i)=><div key={i} className="vac-blk">
                ...
                <div className="g2">
                  ...
                  <F label="Return date" he="תאריך חזרה" err={E(`vac_${i}_ret`)}>
              ...
              {(()=>{
                ...
                return <F label="When was the last date you received Recuperation (Avra'a) payment?" he="מתי התקבל לאחרונה תשלום הבראה?" hint={lessThan1Year ? "Not applicable — less than 1 year of employment" : "Leave empty if you never received it"} err={E('recuperationDate')} style={{marginTop:14}}>
            ...
            {page===3&&<>
              ...
              <F label="Which holidays do you celebrate?" he="אילו חגים את/ה חוגג/ת?" req err={E('holidayType')}>
                <select {...inp('holidayType')} value={f.holidayType} onChange={e=>set('holidayType',e.target.value)}>
                  ...
                  {holidayTypeOpts.map(o=><option key={o.v} value={o.v}>{o.l} / {o.he}</option>)}
                </select>
              </F>
              <F label="When was the last date you received holiday payment?" he="מתי התקבל לאחרונה תשלום עבור חגים?" hint="Leave empty if you never received it" err={E('holidaysDate')} style={{marginTop:12}}>
